...
if (Test-Path $localRtkExe) {
    ...
    try {
        ...
        Write-Host "  Claude Code CLI                     : rtk init -g"
        Write-Host "  VS Code Copilot + GitHub Copilot CLI: Copy-Item .agents\skills\rtk-token-killer\settings-vscode.json .github\hooks\rtk_settings.json"
        exit 0
    } catch {
