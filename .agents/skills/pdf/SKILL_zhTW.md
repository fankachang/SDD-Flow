---
name: pdf
description: 完整的 PDF 操作工具包，支援文字與表格擷取、建立新 PDF、合併/分割文件及處理表單。當 Claude 需要填寫 PDF 表單，或以程式方式大規模處理、生成或分析 PDF 文件時使用。
...
本指南涵蓋使用 Python 函式庫和命令列工具進行的基本 PDF 處理操作。進階功能、JavaScript 函式庫及詳細範例請參閱 reference.md。若需要填寫 PDF 表單，請閱讀 forms.md 並依照其指示操作。
...
c.drawString(100, height - 120, "這是用 reportlab 建立的 PDF")
...
body = Paragraph("這是報告的內容。" * 20, styles['Normal'])
story.append(body)
...
- 進階 pypdfium2 用法請見 reference.md
- JavaScript 函式庫（pdf-lib）請見 reference.md
- 若需要填寫 PDF 表單，請依照 forms.md 的指示操作
- 疑難排解指南請見 reference.md
