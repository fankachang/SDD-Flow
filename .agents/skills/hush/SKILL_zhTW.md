...
name: hush
description: >
  本機、對 AI agent 安全的 per-worktree 密鑰管理工具（macOS 限定）。以 age 將 .env
  加密儲存於 repo 外，僅在 hush run 子行程中注入明文，防止 AI agent 透過 cat/grep
  讀取 secret。觸發詞：hush、密鑰管理、secret 管理、.env 安全。
license: MIT
