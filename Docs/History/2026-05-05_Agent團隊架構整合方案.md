...
> 歷史記錄：本文保留 2026-05-05 當時的角色數量與設計背景，不作為執行期規則。現行規範以 [`Docs/governance/TEAM_COORDINATION_GUIDE.md`](../governance/TEAM_COORDINATION_GUIDE.md) 與各 `*.agent.md` 為準。
...
### 衝突 7：linq-expert & sql-optimizer 的領域區分
...
graph TB
    ...
    subgraph "設計 Phase 2"
        SA["system-architect<br/>技術設計<br/>speckit.plan<br/>SDD 文件支援"]
